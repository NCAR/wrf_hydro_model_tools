README - RAP data regridding scripts for WRF-Hydro


PURPOSE

Scripts to regrid RAP data to a WRF-Hydro domain using the ESMF
...
Acquire RAP data from NOAA's data server: 
http://www.ftp.ncep.noaa.gov/data/nccf/com/rap/prod
For more information regarding RAP see here:
...
1. Create regridding 'weight' files required by the ESMF regridders. The weight
   ...
   weight file is generated by running the 'RAP2WRFHydro_generate_weights.ncl' 
...
2. The second step is to then do the regridding using the 
   'RAP2WRFHydro_regrid.ncl' script which ingests both source data and the 
...
RAP2WRFHydro_regrid.ncl

Input file requirements: 
1. A geogrid file (geo_em.d0x.nc) for your 'destination' domain (WRF-Hydro grid)

2. RAP data (this contains the 'source' domain or grid)  

The RAP data fields used by WRF-Hydro / the regridding scripts are:
