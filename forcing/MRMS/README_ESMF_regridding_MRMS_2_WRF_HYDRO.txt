README_ESMF_regridding_MRMS_2_WRF_HYDRO


PURPOSE:
Utilities to regrid MRMS data to a smaller, completely encompassed WRF-Hydro domain. There are many ways to do this but the tools used here
...
The weight file is generated by running the 'ESMF_genWgts_RADAR2HYDRO_forcing.ncl' script. User's will need
to input the source and destination grid filenames into the 'ESMF_genWgts_RADAR2RFHYDRO_forcing.ncl' program.


2. The second step is to then do the regridding using the 'radar2Hydro_forcing_ESMFregrid_1hr.ncl' script which
...
ESMF_genWgts_RADAR2HYDRO_forcing.ncl

and

radar2Hydro_forcing_ESMFregrid_1hr.ncl


Requirements: To run these scripts you will need the following:
1. a geogrid file from your 'destination' domain

2. your MRMS data from its 'source' domain or grid.  At a minimum for WRF-Hydro you will need
the following MRMS fields:
...
#######################################################################################
Command line structure for running 'ESMF_genWgts_RADAR2HYDRO_forcing.ncl':
#######################################################################################

; Usage:     ncl 'interp_opt="conserve"' 'srcGridName="sourcefile"  'dstGridName="destinationfile"' ESMF_genWgts_RADAR2HYDRO_forcing.ncl
;
...
NOTES on the use of 'ESMF_genWgts_RADAR2HYDRO_forcing.ncl':
...
#######################################################################################
Command line structure for running 'radar2HYDRO_forcing_ESMFregrid.ncl':
#######################################################################################
...
;   
;  ncl 'RUNNAME="RADAR2HYDRO"' 'wrf_domain_name="d01"' 'dstGridName="geo_dst.nc"' 'wgtFileName_in="./RADAR2HYDRO_d01_weight_bilinear.nc"' ./radar2Hydro_forcing_ESMFregrid_1hr.ncl 
;

NOTES on the use of radar2Hydro_forcing_ESMFregrid_1hr.ncl:
1. This script will use the regridding weight files created by the 'ESMF_genWgts_RADAR2HYDRO_forcing.ncl' script. Therefore, do not change the names of those files
or else this script will not be able to find them.
2. The source data to be regridded (MRMS data in netcdf format) needs to be placed in a local directory called 'input_files/'
